...
const feedIdExists = `-- name: FeedIdExists :one
SELECT id
FROM feeds
WHERE id = $1
`

func (q *Queries) FeedIdExists(ctx context.Context, id uuid.UUID) (uuid.UUID, error) {
	row := q.db.QueryRowContext(ctx, feedIdExists, id)
	err := row.Scan(&id)
	return id, err
}
